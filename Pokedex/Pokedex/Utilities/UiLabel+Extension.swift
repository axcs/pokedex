...
extension UILabel {
    ...
    func changeColor()
    {
        ...
        identifyType("Grass", attributedString: attributedString, hexCode: 0x33cc33)
    ...
    func identifyType(_ type: String, attributedString: NSMutableAttributedString, hexCode: Int)
    {
        if (self.text?.range(of: type) != nil)
            {
                let range = (self.text! as NSString).range(of: type)
            attributedString.addAttribute(NSAttributedString.Key.foregroundColor, value: UIColor(netHex: hexCode), range: range)
